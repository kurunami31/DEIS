...
export default function MedicalRecords() {
  ...
  const [form, setForm] = useState({ studentId: '', type: 'DRUG_TEST', result: 'PASSED', testedAt: new Date().toISOString().slice(0, 10), details: '' });
  ...
  useEffect(() => { load(); }, [page, filterType, filterResult]);

  const openCreate = () => {
    setEditing(null);
    setForm({ studentId: '', type: 'DRUG_TEST', result: 'PASSED', testedAt: new Date().toISOString().slice(0, 10), details: '' });
    setShowForm(true);
  };

  const openEdit = (r) => {
    setEditing(r);
    setForm({ studentId: r.studentId, type: r.type, result: r.result, testedAt: r.testedAt.slice(0, 10), details: r.details || '' });
    setShowForm(true);
  };
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, testedAt: new Date(form.testedAt) };
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="font-semibold text-slate-700">{total}</span> medical records
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <CustomSelect value={filterType} onChange={setFilterType} options={[{ value: '', label: 'All types' }, ...TEST_TYPES]} />
      ...
      <section className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="py-10 text-center text-sm text-slate-400">No records found.</td></tr>
              ) : filtered.map((r) => (
                <tr key={r.id}>
                  <td>
                    <p className="font-medium">{r.student.firstName} {r.student.lastName}</p>
                    <p className="text-xs text-slate-400">{r.student.studentNo} · {r.student.program?.name}</p>
                  </td>
                  <td className="text-xs">{TEST_TYPES.find((t) => t.value === r.type)?.label}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-md rounded-[15px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-800">{editing ? 'Edit' : 'Add'} Medical Record</h3>
            <form onSubmit={handleSave} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <CustomSelect value={form.type} onChange={(val) => setForm({ ...form, type: val })} options={TEST_TYPES} />
              </div>
              <div>
      ...
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={() => setDeleting(null)}>
          <div className="w-full max-w-sm rounded-[15px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-800">Delete Medical Record</h3>
            <p className="mt-3 text-sm text-slate-600">Are you sure you want to delete this record for <strong>{deleting.student.firstName} {deleting.student.lastName}</strong>?</p>
