...
export default function MedicalTestPage() {
  ...
  const [form, setForm] = useState({ studentId: '', type: 'PHYSICAL_EXAM', result: 'PASSED', testedAt: new Date().toISOString().slice(0, 10) });
  ...
  const load = () => {
    ...
    else params.set('type', 'PHYSICAL_EXAM,VISION_TEST,HEARING_TEST,CHEST_XRAY,BLOOD_TEST');
  ...
  useEffect(() => { load(); }, [filterType, filterResult]);

  const openCreate = () => {
    setEditing(null);
    setForm({ studentId: '', type: 'PHYSICAL_EXAM', result: 'PASSED', testedAt: new Date().toISOString().slice(0, 10) });
    setShowForm(true);
  };

  const openEdit = (r) => {
    setEditing(r);
    setForm({ studentId: r.studentId, type: r.type, result: r.result, testedAt: r.testedAt.slice(0, 10) });
    setShowForm(true);
  };
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="font-semibold text-slate-700">{total}</span> medical test records
        </div>
      ...
      <section className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="py-10 text-center text-sm text-slate-400">No medical test records found.</td></tr>
              ) : filtered.map((r) => (
                <tr key={r.id}>
                  <td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-md rounded-[15px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-800">{editing ? 'Edit' : 'Add'} Medical Test Record</h3>
      ...
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={() => setDeleting(null)}>
          <div className="w-full max-w-sm rounded-[15px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-800">Delete Medical Test Record</h3>
            <p className="mt-3 text-sm text-slate-600">Delete record for <strong>{deleting.student.firstName} {deleting.student.lastName}</strong>?</p>
